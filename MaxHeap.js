 ...
 */
 class MaxHeap {
  ...
  /**
   * Return the top max-heap number or `undefined` if the heap is empty. The heap is not modified.
   * @returns {number | undefined} max number or `undefined`.
   */
  peek() {
  ...
  /**
   * Removes the macx-heap element and returns it. If the heap is empty returns `undefined` and the heap is not modified.
   * @returns {number | undefined} max element
   */
  pop() {
    if (this.#currSize <= 0) return undefined;
    const maxVal = this.#heap[1];
